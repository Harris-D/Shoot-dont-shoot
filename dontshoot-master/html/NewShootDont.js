...
let expName = 'NewShootDont';  // from the Builder filename that created this script
...
flowScheduler.add(text_4RoutineBegin());
flowScheduler.add(text_4RoutineEachFrame());
flowScheduler.add(text_4RoutineEnd());
...
function practice_trialsLoopBegin(practice_trialsLoopScheduler) {
  ...
  practice_trials = new TrialHandler({
    ...
    trialList: 'practice_image_stimuli.xlsx',
  ...
  for (const thisPractice_trial of practice_trials) {
    ...
    practice_trialsLoopScheduler.add(fixationRoutineEachFrame(snapshot));
    practice_trialsLoopScheduler.add(fixationRoutineEnd(snapshot));
    practice_trialsLoopScheduler.add(StaticPicRoutineBegin(snapshot));
    practice_trialsLoopScheduler.add(StaticPicRoutineEachFrame(snapshot));
    practice_trialsLoopScheduler.add(StaticPicRoutineEnd(snapshot));
    ...
    practice_trialsLoopScheduler.add(blank500RoutineBegin(snapshot));
    practice_trialsLoopScheduler.add(blank500RoutineEachFrame(snapshot));
    practice_trialsLoopScheduler.add(blank500RoutineEnd(snapshot));
...
function trialsLoopBegin(trialsLoopScheduler) {
  ...
  for (const thisTrial of trials) {
    ...
    trialsLoopScheduler.add(StaticPicRoutineBegin(snapshot));
    trialsLoopScheduler.add(StaticPicRoutineEachFrame(snapshot));
    trialsLoopScheduler.add(StaticPicRoutineEnd(snapshot));
...
var StaticPicComponents;
function StaticPicRoutineBegin(snapshot) {
  return function () {
    //------Prepare to start Routine 'StaticPic'-------
    t = 0;
    StaticPicClock.reset(); // clock
    frameN = -1;
    ...
    // keep track of which components have finished
    StaticPicComponents = [];
    StaticPicComponents.push(image);
    
    for (const thisComponent of StaticPicComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function StaticPicRoutineEachFrame(snapshot) {
  return function () {
    //------Loop for each frame of Routine 'StaticPic'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
...
function StaticPicRoutineEnd(snapshot) {
  return function () {
...
function trialRoutineEachFrame(snapshot) {
  return function () {
    ...
    if (key_resp_2.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_2.getKeys({keyList: ['space', 'left', 'right'], waitRelease: false});
...
var text_4Components;
function text_4RoutineBegin(snapshot) {
  return function () {
    //------Prepare to start Routine 'text_4'-------
    t = 0;
    text_4Clock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    ...
    // keep track of which components have finished
    text_4Components = [];
    text_4Components.push(text_5);
    text_4Components.push(key_resp_3);
    
    for (const thisComponent of text_4Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function text_4RoutineEachFrame(snapshot) {
  return function () {
    //------Loop for each frame of Routine 'text_4'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
...
function text_4RoutineEnd(snapshot) {
  return function () {
